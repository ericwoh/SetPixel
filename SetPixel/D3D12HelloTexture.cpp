...
void Update(D3D12HelloTexture* d3d, float dt);
...
void D3D12HelloTexture::GenerateTextureData()
{
    const UINT rowPitch = TextureWidth * TexturePixelSize;
    const UINT cellPitch = rowPitch >> 3;       // The width of a cell in the checkboard texture.
    const UINT cellHeight = TextureWidth >> 3;  // The height of a cell in the checkerboard texture.
    const UINT textureSize = rowPitch * TextureHeight;
    m_pixels.resize(textureSize);
    ...
    for (UINT n = 0; n < textureSize; n += TexturePixelSize)
    {
        ...
        if (i % 2 == j % 2)
        {
            m_pixels[n] = rand() % 255;     // R
            m_pixels[n + 1] = rand() % 255; // G
            m_pixels[n + 2] = rand() % 255; // B
            m_pixels[n + 3] = 0xff; // A
        }
        else
        {
...
void D3D12HelloTexture::SetPixel(UINT x, UINT y, UINT8 r, UINT8 g, UINT8 b)
{
    const UINT rowPitch = TextureWidth * TexturePixelSize;
    const UINT cellPitch = rowPitch >> 3;       // The width of a cell in the checkboard texture.
    const UINT cellHeight = TextureWidth >> 3;  // The height of a cell in the checkerboard texture.
...
void D3D12HelloTexture::Clear(UINT8 r, UINT8 g, UINT8 b)
{
    const UINT rowPitch = TextureWidth * TexturePixelSize;
    const UINT cellPitch = rowPitch >> 3;       // The width of a cell in the checkboard texture.
    const UINT cellHeight = TextureWidth >> 3;  // The height of a cell in the checkerboard texture.
...
{
    Update(this, static_cast<float>(m_dt));
}
